                     ...
                     __global bsizes *nslots)
{
  ...
  for (uint32_t block = id; block < NBLOCKS; block += get_global_size(0)) {
    ...
    for (uint32_t i = 0; i < HASHESPERBLAKE; i++) {
	  ...
	  //const uint8_t *ph = hash + i * 64 + j * WN / 8;
...
#if BUCKBITS <= 8
	  const uint32_t bucketid = (uint32_t)(ph[0] >> (8 - BUCKBITS));
#elif BUCKBITS > 8 && BUCKBITS <= 16
	  const uint32_t bucketid = ((uint32_t)ph[0] << (BUCKBITS - 8)) | ph[1] >> (16 - BUCKBITS);
#elif BUCKBITS > 16
	  const uint32_t bucketid = ((((uint32_t)ph[0] << 8) | ph[1]) << (BUCKBITS - 16)) | ph[2] >> (24 - BUCKBITS);
#else
#error not implemented
#endif
